...
def icon(element):
    if "serviceTask" in element.tag:
        if "${httpRequestorTask}" in element.attrib.values():
            return Internet(element.attrib["id"])
        elif (
            "${updateScimUserTask}" in element.attrib.values()
            or "${createScimUserTask}" in element.attrib.values()
        ):
...
if __name__ == "__main__":
    ...
    if args.direction not in ["TB", "BT", "LR", "RL"]:
        print(f'Invalid direction param {args.direction} reverting to default, "TB"')
        args.direction = "TB"
